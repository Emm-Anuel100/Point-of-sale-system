...
## Query to fetch the number of sales made within the current year and month
$sql_sales_count = "SELECT COUNT(*) AS sales_count FROM sales WHERE YEAR = '$current_year' AND MONTH = '$current_month'";
...
if ($result_sales_count->num_rows > 0) {
    ...
    echo "<h2>" . number_format($sales_count) . " <span>sales made this month</span></h2><br/>";
   } else {
      echo "<h2>0 <span>sales made this month</span></h2><br/>";
   }

## Query to fetch the sum of prices from the total_naira column for the current year and month
$sql_total_naira_sum = "SELECT SUM(total_naira) AS total_naira_sum FROM sales WHERE YEAR = '$current_year' AND MONTH = '$current_month'";
